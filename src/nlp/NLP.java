...
//	private static final String[] unuseful = {"c", "df", "e", "f", "h", "mq", "o", "p", "q", "r", "rr", "rz", "u", "vd", "vi", "vq", "y"};
	...
	private static int total;																		//最终的分词个数
	private static ArrayList<Word> postWordList;								//最终的分词结果列表
	private static HashMap<String, Integer> word_TFIDF;					//接口函数nlp(String)返回对象，HashMap存储字符串及对应的TF*IDF值的键值对
	...
	public static HashMap<String, Integer> nlp(String weibo) {
		...
		DAG.generateDAG(weibo);								//生成有向无环图DAG
		DAG.getLongestPath();										//计算词频最大分词路径
		DAG.partition();													//进行语句划分
		DAG.merge(postWordList);								//将预分词结果进行进一步的短词语合并
		
		filter(0);																//过滤所有单个字符（包括中文字符和所有非中文字符）
		total = postWordList.size();								//统计当前的分词总个数
		filter(1);																//进一步过滤不满足词性要求的字符
		
		TF_IDF();															//执行TF-IDF算法
		return word_TFIDF;											//返回HashMap<String, Integer>对象
	}
	...
	private static void TF_IDF() {
		for(Word word : postWordList) {
			...
			int IDF = (int) (Dictionary.getIDF(thisWord) / total * 10000);					//int和double之间的转换因子为10000
