...
public class DAG {
	...
	// 中文语句预分词结果列表
	private static ArrayList<Word> preWordList;
	...
	public static void generateDAG(String weibo) {
		...
		for (int i = 0; i < n - 1; i++) {
			...
			for (int j = i + 1; j < n; j++) {
				// 从i到j截取weibo字符串
				String subString = weibo.substring(i, j + 1);
				// 在预设字典中查询词组，得到Word类对象word
				Word word = Dictionary.search(subString);
